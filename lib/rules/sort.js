 ...
 * @param {Array} imports - Array of import objects to
 ...
 * ASC: external (-1) first, then deeper (../../ = 2), then shallower (./ = 0)DESC: shallow (./ = 0) first, then deeper (../../ = 2), then external (-1) last
...
function getPriority(depth, direction) {
	if (direction === SortDirection.ASC) {
		// External gets highest priority (lowest number)
		// Then sort by depth descending (2, 1, 0)
		return depth === -1 ? -1000 : -depth;
	}
	// DESC: shallow paths first, external last
	return depth === -1 ? 1000 : depth;
}
 ...
 * @param {Array} imports - Array of import objects to group
 * @param {string} direction - Sort direction
